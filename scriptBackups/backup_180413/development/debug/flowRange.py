	...
	if state == IDLE_STATE:						#check database for profile data & decide which state to move to
		...
		tStart = datetime.datetime.now()                      #time out setup
                ...
                while isProfiled == 1:
                        ...
                        if(tNow > tEnd):                            #on time out check if the system is profiled
			...
			if isProfiled == 1 and cnt == PULSES_PER_LITRE:
				...
				tElapsed = (tLitreEnd - tStart)				#calculate the read time
				timeRow = divmod(tElapsed.total_seconds(), 60)		#divmod returns quotient and remainder
				tLitreRead = (timeRow[0] * 60) + timeRow[1]		#multipy quotient by sixty and add remainder
